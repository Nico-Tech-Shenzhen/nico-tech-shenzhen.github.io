...
Robot
Kit-Blue」**](http://makeblock.com/ultimate-robot-kit-blue)の何かをつかむ機構をクレーンでつり下げたら、UFOキャッチャーのようなものを作ることができる。作ったことがある機構、どうやって実現できるかの仕組みが作り手に蓄積されるたびにできることが広がっていく。

### 新しいキットを生み出すハッカソン
Makeblockでは、Makeblockを用いたハッカソンを頻繁に行っている。下の動画は連載の第一回で触れた[**Seeed**](https://takasumasakazu.net/fabcross/diy-2016-02-23/)のエンジニアと一緒に行ったハッカソンで、36時間で「実用的なもの（pragmatism）」を作るのがテーマだ。テーマは「アート」「楽器」などそのたびに変わる。
...
Music Robot Kit
V2.0」**](http://makeblock.com/music-robot-kit-v2-0-with-electronics)はハッカソンからの派生だそうだ。
